...
M1 已完成分步规划，M1-A 已实现并通过自动化验证，交互式收口验收仍待完成；后续切片必须按以下顺序独立验收，详细边界见 [`M1_PLAN.md`](M1_PLAN.md)：
...
M0 技术验收已经完成并提交。项目当前处于 Developer Preview 的 M1 Windows 平台能力开发阶段；M1-A 已实现平台中立契约、可见顶层窗口发现、终端用户明确选择界面和独立受控测试窗口，并通过自动化验证，但交互式收口验收仍待完成。M1-B 至 M1-G 尚未实现，当前仍没有真实游戏窗口捕获、真实输入控制或 GTA 自动操作能力。

下一项最小实施任务固定为 M1-B“窗口与进程身份重新验证”：基于 M1-A 的 `WindowSelection` 和 Platform.Windows 仅内存原生映射，重新验证窗口、PID、进程启动身份、窗口类和可执行程序身份。Capture Target 与 Input Target 仍须分离，且不得提前实现捕获、armed 或输入。

后续切片必须按照 M1-B → M1-C → M1-D → M1-E → M1-F → M1-G 的顺序推进；每次推送前继续执行公开仓库检查清单。代码许可证只能由项目维护者决定，在添加 `LICENSE` 前不得称为开源项目或接受需要合并代码的外部贡献。
