...
状态：规划稿，不代表任何 Windows 平台能力已经实现。  
适用基线：`5f932d10bd8b7d6123ae2c6e03b1a4f43c17ae35`  
权威边界：[`AGENTS.md`](../AGENTS.md)、[`ARCHITECTURE.md`](ARCHITECTURE.md) 和本轮项目维护者批准的 M1 范围。

M0 技术验收已经完成并提交；项目当前处于 M1 Windows 平台能力开发准备阶段。M1-A 至 M1-G 均尚未实现，当前没有真实游戏窗口捕获、真实输入控制或 GTA 自动操作能力。
...
本次同步只修正文档状态和边界，不表示 M1-A 或任何 Windows 平台能力已经开始实现。

## 17. 下一最小实施切片：M1-A 精确范围

M1-A 建议拆成两个可独立提交的最小步骤，但作为同一验收切片完成：
